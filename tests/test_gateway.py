...
def _gateway_post(headers=None):
    return client.post(
        "/v1/gateway/verify",
        json={
            "workflow_id": "finance-briefing",
...
def test_gateway_rejects_anonymous_request(monkeypatch):
    monkeypatch.setattr(api_main, "_run_claim_assessment", _fake_assessment)
    monkeypatch.setattr(api_main, "save_report", lambda assessment: assessment)
    ...
    assert response.json()["detail"]["code"] == "gateway_api_key_required"


def test_gateway_rejects_session_authenticated_researcher_without_key(monkeypatch):
    # A logged-in researcher-tier user (which grants api_access) must NOT reach
    # the gateway via their web session. Only an explicit gateway:write API key.
    monkeypatch.setattr(
        api_main,
        "_profile_from_request",
        lambda request: (
            api_main.AuthContext(owner_id="session-user", auth_method="supabase_jwt", email="contract@example.com"),
            _researcher_profile("session-user"),
        ),
    )
    monkeypatch.setattr(api_main, "_run_claim_assessment", _fake_assessment)
    monkeypatch.setattr(api_main, "save_report", lambda assessment: assessment)
...
def test_gateway_rejects_api_key_missing_scope(tmp_path, monkeypatch):
    monkeypatch.setenv("EVIDRAI_GATEWAY_AUDIT_STORE", str(tmp_path / "gateway_audit.jsonl"))
    monkeypatch.setattr(api_main, "authenticate_api_key", lambda key: ApiKeyRecord(key_id="key_2", owner_id="contract-user", scopes=["assessments:write"]))
    monkeypatch.setattr(api_main, "get_or_create_profile", lambda owner_id, email="": _researcher_profile(owner_id))
    monkeypatch.setattr(api_main, "_run_claim_assessment", _fake_assessment)
    monkeypatch.setattr(api_main, "save_report", lambda assessment: assessment)

    response = _gateway_post(headers={"X-Evidrai-Api-Key": "evd_live_no_gateway_scope"})
