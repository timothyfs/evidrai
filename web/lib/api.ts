...
async function request<T>(path: string, init?: RequestInit, options?: { sameOrigin?: boolean }): Promise<T> {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      const supabaseDetail = typeof detail === 'object' && detail?.supabase_detail ? detail.supabase_detail : null;
      const upstreamNested = typeof upstreamDetail === 'object' && upstreamDetail?.detail ? upstreamDetail.detail : null;
      const supabaseNested = typeof supabaseDetail === 'object' && supabaseDetail?.detail ? supabaseDetail.detail : supabaseDetail;
      const upstreamMessage = typeof upstreamNested === 'string' ? upstreamNested : (typeof upstreamNested === 'object' ? upstreamNested?.message : '');
      const supabaseMessage = typeof supabaseNested === 'string' ? supabaseNested : (typeof supabaseNested === 'object' ? supabaseNested?.message || supabaseNested?.msg || supabaseNested?.error_description : '');
      const upstreamCode = typeof upstreamNested === 'object' ? upstreamNested?.code : '';
      const supabaseCode = typeof supabaseNested === 'object' ? supabaseNested?.code || supabaseNested?.error_code || supabaseNested?.error : '';
      const detailCode = typeof detail === 'object' ? detail?.code : '';
      const fallback = typeof detail === 'object' && detail?.fallback ? ` ${detail.fallback}` : '';
      const baseMessage = upstreamMessage || supabaseMessage || detail?.message || payload?.error || message;
      const diagnosticCode = upstreamCode || supabaseCode || detailCode;
